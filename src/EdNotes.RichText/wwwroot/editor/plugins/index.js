...
export function buildToolbarLayout(pluginList, toolbarString) {
    ...
    return groups.map(group => group
        ...
        .filter(btn => btn && (btn.plugin === 'core' || enabled.has(btn.plugin)))
...
registerPlugin('formatting', {
    name: 'formatting',
    buttons: [
        { name: 'bold', text: 'B', label: 'Bold', command: 'strong', shortcut: 'Ctrl+B' },
        { name: 'italic', text: 'I', label: 'Italic', command: 'em', shortcut: 'Ctrl+I' },
        { name: 'underline', text: 'U', label: 'Underline', command: 'u', shortcut: 'Ctrl+U' },
        { name: 'removeformat', text: '⌫', label: 'Clear Formatting', command: 'format:clear' }
    ]
});
...
registerPlugin('tables', {
    name: 'tables',
    buttons: [
        { name: 'table', text: '⌗', label: 'Insert Table', command: 'table:insert' }
    ]
});
