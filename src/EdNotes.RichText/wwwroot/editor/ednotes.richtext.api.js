...
const EdNotesRichText = {
  ...
  init(config = {}) {
    ...
    // Validate plugins
    const requestedPlugins = finalConfig.plugins
      .split(/\s+/)
      .filter((p) => p.length > 0);
    ...
    if (invalidPlugins.length > 0) {
      console.warn(
        `[EdNotes.RichText] Unknown plugins: ${invalidPlugins.join(", ")}`
      );
    }

    const toolbarLayout = buildToolbarLayout(
      requestedPlugins,
      finalConfig.toolbar
    );
    ...
    const matchedInstances = Array.from(elements)
      .map((el) => allInstances.find((instance) => instance.textarea === el))
      .filter(Boolean);
